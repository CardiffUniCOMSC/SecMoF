...
public class ParagonImpl extends MinimalEObjectImpl.Container implements Paragon {
	...
	private double computeORprobability(EList<Paragon> paragons) {
		// A method to compute probability of OR paragons
		double probabilityComputed = 1;
		for (Paragon paragon : paragons) {
			double paragonProb = paragon.getProbability();
			probabilityComputed = probabilityComputed * (1 - paragonProb);
		}
		probabilityComputed = 1 - probabilityComputed;
		return probabilityComputed;
	}

	private double computeANDprobability(EList<Paragon> paragons) {
		// A method to compute probability of AND paragons
		double probabilityComputed = 1;
		for (Paragon paragon : paragons) {
			double paragonProb = paragon.getProbability();
			probabilityComputed = probabilityComputed * paragonProb;
		}
		return probabilityComputed;
	}
